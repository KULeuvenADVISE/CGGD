...
This script is a combination of some functions of https://github.com/LIVIAETS/extended_logbarrier/blob/master/losses.py and 
https://github.com/LIVIAETS/SizeLoss_WSS/blob/master/losses.py.

No changes were made to the corresponding functions except that certain functions were omitted.
    ...
    def __call__(self, probs: Tensor, _: Tensor, __: Tensor,
        ...
        for b in range(B):
            for k in self.idc:
                ...
                init = torch.zeros((), dtype=torch.float32, requires_grad=probs.requires_grad, device=probs.device)
                sublosses.append(reduce(add, (self.penalty(v) for v in shifted), init))
