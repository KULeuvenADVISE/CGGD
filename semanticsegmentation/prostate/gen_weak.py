...
def centroid_strat(orig_mask: np.ndarray, filename: str, filling: int) -> Tuple[np.ndarray, int]:
    ...
    if size:  # Positive images
        ...
        masked_res: Image.Image = Image.fromarray(np.einsum("hw,wh->wh", np.array(res_img), orig_mask), mode='L')
...
def random_strat(orig_mask: np.ndarray, filename: str, filling: int) -> Tuple[np.ndarray, int]:
    ...
    if size:  # Positive images
        ...
        # Remove overflow if needed
        masked_res: Image.Image = Image.fromarray((np.einsum("hw,wh->wh", np.array(res_img), orig_mask)).astype(np.uint8),
                                                  mode='L')
...
def weaken_img(pn: Tuple, strategy: Callable) -> Tuple[int, int]:
    ...
    # Do the magic
    if args.per_connected_components:
        blobs: np.ndarray
        n_blob: int
        blobs, n_blob = measure.label(ni, background=0, return_num=True)
        assert set(np.unique(blobs)) == set(range(n_blob + 1)), np.unique(blobs)

        _res_arr: np.ndarray = np.zeros_like(ni, dtype=np.uint8)
        for id_blob in range(1, n_blob + 1):
            masked_blob: np.ndarray = blobs == id_blob

            partial_res_img, partial_size = strategy(masked_blob, n, filling)
            _res_arr |= np.array(partial_res_img) == filling

        res_img = Image.fromarray(_res_arr * filling, mode='L')
        size = (_res_arr == filling).astype(np.int64).sum()  # Should improve later ; quite ugly
    else:
...
def get_args() -> Namespace:
    ...
    parser.add_argument("--per_connected_components", action="store_true")
