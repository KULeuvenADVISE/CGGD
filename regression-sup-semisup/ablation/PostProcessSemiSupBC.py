    ...
    if loss_boolean:
        ...
        for i in range(0, len(train_sizes)):
            ...
            for j in range(0, number_weight_initializations):
                for k in range(0, number_of_dataset_realisations):
                    with open(file + '_' + str(j) + '_dataset_' + str(k) + '_percentage_' + str(train_sizes[i]) + '_rescale_factor_' + str(r) + '.pickle',
                              'rb') as handle:
        ...
        for r in rescale_factors[1:]:
            ...
            for i in range(0, len(train_sizes)):
                ...
                for j in range(0, number_weight_initializations):
                    for k in range(0, number_of_dataset_realisations):
                        with open(file + '_' + str(j) + '_dataset_' + str(k) + '_percentage_' + str(train_sizes[i]) + '_rescale_factor_' + str(r) + '.pickle', 'rb') as handle:
                            loaded_results_con = pickle.load(handle)
                            best_epoch = loaded_results_con['best_epoch']
                            temp_results.append(loaded_results_con['loss_value_test'])
                all_results.append(temp_results)
...
rescale_factors = [1.2, 1.5, 5, 20, 500]
exp_latex_code_loss_bias_train, exp_latex_code_satisfaction_bias_train, results_loss_bias_train, results_sr_bias_train = \
    construct_results_train_rescale_factors(name_experiment='BiasCorrection_SemiSup_DiffRescaleFactors',
...
used_train_sizes = [0, 1, 2, 3]
rescale_factors = [1.2, 1.5, 5, 20, 500]
exp_latex_code_loss_bias_val, exp_latex_code_satisfaction_bias_val, results_loss_bias_val, results_sr_bias_val = \
    construct_results_val_rescale_factors(name_experiment='BiasCorrection_SemiSup_DiffRescaleFactors',
                                          name_dataset='Sample_Bias_correction_ucl_reduced',
                                          train_sizes=[str(sizes) for sizes in used_train_sizes],
                                          rescale_factors=rescale_factors,
                                          directory_experiment='./Results',
...
used_train_sizes = [0, 1, 2, 3]
rescale_factors = [1.2, 1.5, 5, 20, 500]
exp_latex_code_loss_bias_test, exp_latex_code_satisfaction_bias_test, results_loss_bias_test, results_sr_bias_test = \
    construct_results_test_rescale_factors(name_experiment='BiasCorrection_SemiSup_DiffRescaleFactors',
                                           name_dataset='Sample_Bias_correction_ucl_reduced',
                                           train_sizes=[str(sizes) for sizes in used_train_sizes],
                                           rescale_factors=rescale_factors,
                                           directory_experiment='./Results',
