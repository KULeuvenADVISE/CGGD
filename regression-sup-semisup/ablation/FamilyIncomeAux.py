...
def perform_experiment_different_training_sizes_different_rescale_factors(directory_dataset, name_dataset, extension_dataset, initial_learning_rate,
                                                                          ...
                                                                          name_experiment, margin, rescale_factors):
                ...
                for i in range(0, len(weight_states)):
                    ...
                    if constrained:
                        trained_model, train_loss_results, train_metric_results, train_satisfaction_results, val_loss_results, val_metric_results, val_satisfaction_results, epoch, list_lr, list_lr_loss, list_lr_con, timing, best_epoch = \
                            cggd.train_model_adaptive_rescale_factor(model=model, input_dim=input_dim, output_dim=output_dim, optimizer=optimizer,
                                                                     list_bound_constraints=list_bound_constraints,
                                                                     train_number=np.size(x_train, 0), train_dataset=train_dataset,
                                                                     train_batch_size=batch_size, val_number=np.size(x_val, 0),
                                                                     val_dataset=val_dataset, val_batch_size=batch_size, max_epochs=max_num_epochs,
                                                                     max_con_epochs=max_epoch_constraints, rescale_constant=r,
        ...
        for k in range(0, len(dataset_states1)):
            for r in range(0, len(percentages_unsupervised)):
                ...
                for i in range(0, len(weight_states)):
                    ...
                    model.save_weights(
                        filepath=save_location_model + 'weights_con_model_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r) + '/weights_con_model_initial_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r) + '_rescale_factor_' + rescale_factor_string)
