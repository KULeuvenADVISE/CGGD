...
def sum_expression_output_variables(signed_indicator, predictions):
    """This function preforms the sum over the relevant output variables.

    In other words, this function can be used in the GradientTape such that the sum expression can be automatically differentiated.
    ...
    sum_bound_constraint = tf.math.reduce_sum(signed_indicated_predictions, axis=1)

    return sum_bound_constraint


def check_sum_expression(expression, type_constraint, bound, signed_indicator, batch_size):
    ...
    satisfaction_signed_indicator = tf.math.multiply(unsatisfied_constraint, signed_indicator)
...
def compute_sum_grad(model, inputs, targets, batch_size, output_dim, bound_sm_eq, bound_sm_st, bound_la_eq, bound_la_st,
                     ind_sm_eq, ind_sm_st, ind_la_eq, ind_la_st, signed_indicator, rescale_constant=1.5):
    ...
    grad_la_st : list[tf.Tensor]
        ...
        variables of model
    grad_sum : list[tf.Tensor]
        list of tensors corresponding to the gradient of the unsatisfied sum constraint with respect to the trainable variables of model
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        with tape.stop_recording():
            metric_value = tf.keras.losses.mean_absolute_error(targets, tape_pred)  # the metric does not need to be differentiated and thus not necessary to record
            ...
            sum_expression = sum_expression_output_variables(signed_indicator, tape_pred)

            unsatisfied_sum_constraint, satisfaction_signed_indicator, counter_unsatisfaction_sum, counter_total_sum = \
            ...
            count_con_total = [count_sm_eq, count_sm_st, count_la_eq, count_la_st, counter_total_sum]
            # put total number of unsatisfied constraints for each type in a list
            count_con_unsat = [count_unsat_sm_eq, count_unsat_sm_st, count_unsat_la_eq, count_unsat_la_st, counter_unsatisfaction_sum]
        ...
        unsatisfied_sum = perform_satisfaction_expression(tf.math.multiply(tf.math.multiply(rescale_constant, grad_loss_output_norm), satisfaction_signed_indicator), tape_pred)
    ...
    # compute gradient for the sum constraint
    grad_sum = tape.gradient(unsatisfied_sum, model.trainable_variables, unconnected_gradients=tf.UnconnectedGradients.ZERO)
    ...
    return grad_loss, grad_sm_eq, grad_sm_st, grad_la_eq, grad_la_st, grad_sum, loss_value, metric_value, sat_ratio, grad_loss_output_norm


def unsup_compute_sum_grad(model, inputs, targets, batch_size, output_dim, bound_sm_eq, bound_sm_st, bound_la_eq, bound_la_st,
                           ind_sm_eq, ind_sm_st, ind_la_eq, ind_la_st, signed_indicator, grad_loss_output_norm, rescale_constant=1.5):
    """Compute the gradients of the unsatisfied constraints for an unsupervised batch in semi-supervised learning.
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        loss_value = tf.keras.losses.mean_squared_error(targets, tape_pred)
        with tape.stop_recording():
            metric_value = tf.keras.losses.mean_absolute_error(targets, tape_pred)
            # check satisfaction for smaller or equal constraints
            ind_sat_sm_eq, count_unsat_sm_eq, count_sm_eq = determine_sat_con(tape_pred,
                                                                              bound_sm_eq,
                                                                              batch_size,
    ...
    return grad_sm_eq, grad_sm_st, grad_la_eq, grad_la_st, grad_sum, loss_value, metric_value, sat_ratio


def compute_fuzzy_grad(model, inputs, targets, batch_size, output_dim, bound_sm_eq, bound_sm_st, bound_la_eq, bound_la_st, ind_sm_eq,
                       ind_sm_st, ind_la_eq, ind_la_st, signed_indicator):
    """Compute gradient of the fuzzy-logic loss function.

    The code presented here is not the most efficient one, but it allows for computing the satisfaction ratio in completely the same fashion as for
    the unconstrained model and the CGGD model.
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        loss_smaller_equal = tf.math.reduce_sum(tf.math.reduce_sum(tf.math.maximum(1.1 - tape_pred, 0), axis=1))
        loss_larger_equal = tf.math.reduce_sum(tf.math.reduce_sum(tf.math.maximum(tape_pred + 0.1, 0), axis=1))
        loss_sum_constraint = tf.math.reduce_sum(tf.math.maximum(tf.math.reduce_sum(tape_pred*tf.constant([-1, 1], dtype=tf.float32), axis=1), 0))

        combined_loss = loss_value + loss_smaller_equal + loss_larger_equal + loss_sum_constraint
...
def unsup_compute_fuzzy_grad(model, inputs, targets, batch_size, output_dim, bound_sm_eq, bound_sm_st, bound_la_eq, bound_la_st, ind_sm_eq,
                       ind_sm_st, ind_la_eq, ind_la_st, signed_indicator):
    """Compute gradient of the fuzzy-logic loss function.
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        combined_loss = loss_smaller_equal + loss_larger_equal + loss_sum_constraint
...
def initialize_indicator_tensors(batch_size, output_dim, list_bound_constraints):
    ...
    input_tensor : list[tf.Tensor]
        list containing all the output tensors and tensors indicating the variables present in the sum and connection constraints.
    ...
    if list_bound_constraints is None or list_bound_constraints == []:
        ...
        # the constraint is T_min <= T_max (so tf.constant(0, dtype=tf.int32) <= tf.constant(1, dtype=tf.int32))
        sign_ind_tmin_tmax = sum_output_variables(index_positive_output=[tf.constant(1, dtype=tf.int32)],
                                                  index_negative_output=[tf.constant(0, dtype=tf.int32)],
                                                  batch_size=batch_size,
                                                  output_dim=output_dim)

        input_tensor = [bound_se, bound_ss, bound_le, bound_ls, bound_ind_se, bound_ind_ss, bound_ind_le, bound_ind_ls, sign_ind_tmin_tmax]

        return input_tensor


def validation_check(model, inputs, targets, batch_size, output_dim, bound_sm_eq, bound_sm_st, bound_la_eq, bound_la_st,
                     ind_sm_eq, ind_sm_st, ind_la_eq, ind_la_st, signed_indicator):
    ...
    # added the satisfaction check for the sum constraint
    sum_expression = sum_expression_output_variables(signed_indicator, predictions)
...
def train_model(model, input_dim, output_dim, optimizer, list_bound_constraints, train_number, train_dataset, train_batch_size,
    ...
    while epoch <= max_epochs:
        ...
        for x, y in train_dataset:
            ...
            if current_batch_size == train_batch_size:
                # optimize model
                gradient_loss, gradient_smaller_equal, gradient_smaller_strict, gradient_larger_equal, gradient_larger_strict, gradient_sum_constraint, train_loss_value, train_metric_value, train_satisfaction_ratio, grad_loss_output_norm1 \
                    = compute_sum_grad(model=model,
                                       inputs=x,
                                       targets=y,
                                       ...
                                       signed_indicator=ind_tensors_train[8])
                ...
                # optimize model
                gradient_loss, gradient_smaller_equal, gradient_smaller_strict, gradient_larger_equal, gradient_larger_strict, gradient_sum_constraint, train_loss_value, train_metric_value, train_satisfaction_ratio, grad_loss_output_norm2 \
                    = compute_sum_grad(model=model,
                                       inputs=x,
                                       targets=y,
                                       batch_size=current_batch_size,
                                       ...
                                       signed_indicator=ind_tensors_train_last[8])
            ...
            if current_batch_size == val_batch_size:
                val_loss_value, val_metric_value, val_sat_ratio = \
                    validation_check(model=model,
                                     ...
                                     signed_indicator=ind_tensors_val[8])
                ...
                val_loss_value, val_metric_value, val_sat_ratio = \
                    validation_check(model=model,
                                     ...
                                     signed_indicator=ind_tensors_val_last[8])
...
def train_model_semi_sup(model, input_dim, output_dim, optimizer, list_bound_constraints, train_number, train_dataset,
                         ...
                         save_location_model, sup_batches, save_file_model):
    """Train the given model.
    ...
    while epoch <= max_epochs:
        ...
        for x, y in train_dataset:
            ...
            if counter_batch in sup_batches:
                ...
                if current_batch_size == train_batch_size:
                    # optimize model
                    gradient_smaller_equal, gradient_smaller_strict, gradient_larger_equal, gradient_larger_strict, gradient_sum_constraint, train_loss_value, train_metric_value, train_satisfaction_ratio \
                        = unsup_compute_sum_grad(model=model,
                                                 inputs=x,
                                                 targets=y,
                                                 batch_size=current_batch_size,
                                                 output_dim=output_dim,
...
def train_model_unconstrained(model, optimizer, train_number, train_dataset, train_batch_size, val_number, val_dataset, val_batch_size,
                              max_epochs, learning_rate, max_constant_perf, save_location_model, input_dim, output_dim,
                              list_bound_constraints, save_file_model):
    ...
    while epoch <= max_epochs:
        for x, y in train_dataset:
            ...
            if current_batch_size == train_batch_size:
                # optimize model
                gradient_loss, gradient_smaller_equal, gradient_smaller_strict, gradient_larger_equal, gradient_larger_strict, gradient_sum_constraint, train_loss_value, train_metric_value, train_satisfaction_ratio, _ \
                    = compute_sum_grad(model=model,
                                       inputs=x,
                                       targets=y,
                                       batch_size=current_batch_size,
    ...
    return model, train_loss_results, train_metric_results, train_satisfaction_results, val_loss_results, val_metric_results, val_satisfaction_results, epoch, list_lr_loss, timing, best_epoch
...
def train_model_unconstrained_only_label(model, optimizer, train_number, train_dataset, train_batch_size, val_number, val_dataset,
                                         ...
                                         output_dim, list_bound_constraints, sup_batches, save_file_model):
    """Train model without constraints.
...
def train_model_fuzzy(model, optimizer, train_number, train_dataset, train_batch_size, val_number, val_dataset, val_batch_size,
                      max_epochs, learning_rate, max_constant_perf, save_location_model, input_dim, output_dim,
                      list_bound_constraints, save_file_model):
    """Train the model with a fuzzy-logic loss function.
    ...
    while epoch <= max_epochs:
        ...
        print("Epoch {:03d}: Loss: {:.8f}, Mean absolute error: {:.8f}, Satisfaction rate val: {:.8%}, Satisfaction rate train: {:.8%}.".format(
            epoch,
            epoch_val_loss_average.result(),
            epoch_val_metric_average.result(),
            epoch_val_satisfaction_average.result(),
...
def train_model_semi_sup_fuzzy(model, optimizer, train_number, train_dataset, train_batch_size, val_number, val_dataset, val_batch_size,
                               max_epochs, learning_rate, max_constant_perf, save_location_model, input_dim, output_dim,
                               list_bound_constraints, save_file_model, sup_batches):
    """Train the model with a fuzzy-logic loss function.
        ...
        for x, y in train_dataset:
            if counter_batch in sup_batches:
                ...
                if current_batch_size == train_batch_size:
                    # optimize model
                    gradient_loss, train_loss_value, train_metric_value, train_satisfaction_ratio \
                        = unsup_compute_fuzzy_grad(model=model,
                                                   inputs=x,
                                                   targets=y,
                                                   batch_size=current_batch_size,
                                                   output_dim=output_dim,
        ...
        if current_batch_size == test_batch_size:
            test_loss_value, test_metric_value, test_sat_ratio = \
                validation_check(model=model,
                                 ...
                                 signed_indicator=ind_tensors_test[8])
            ...
            test_loss_value, test_metric_value, test_sat_ratio = \
                validation_check(model=model,
                                 ...
                                 signed_indicator=ind_tensors_test_last[8])
