...
def compute_fuzzy_grad(model, inputs, targets, batch_size, output_dim, bound_sm_eq, bound_sm_st, bound_la_eq, bound_la_st,
                       ind_sm_eq, ind_sm_st, ind_la_eq, ind_la_st, signed_indicator_food_expense, signed_indicator_income,
                       lower_bound_output_variables, upper_bound_output_variables, lower_bound_input_variables,
...
def unsup_compute_fuzzy_grad(model, inputs, targets, batch_size, output_dim, bound_sm_eq, bound_sm_st, bound_la_eq, bound_la_st,
                       ind_sm_eq, ind_sm_st, ind_la_eq, ind_la_st, signed_indicator_food_expense, signed_indicator_income,
                       lower_bound_output_variables, upper_bound_output_variables, lower_bound_input_variables,
                       upper_bound_input_variables, undo_norm_direction):
    """ Train model unsupervised with fuzzy-logic loss function.
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        combined_loss = loss_smaller_equal + loss_larger_equal + loss_income_constraint + loss_food_constraint
...
def train_model(model, input_dim, output_dim, optimizer, list_bound_constraints, train_number, train_dataset, train_batch_size,
    ...
    """Train the given model with CGGD.
...
def train_model_semi_sup(model, input_dim, output_dim, optimizer, list_bound_constraints, train_number, train_dataset,
    ...
    """Train the given model with CGGD semi-supervised.
    ...
    initial_rescale_factor = rescale_constant
...
def train_model_unconstrained(model, optimizer, train_number, train_dataset, train_batch_size, val_number, val_dataset, val_batch_size,
    ...
    while epoch <= max_epochs:
        for x, y in train_dataset:
            ...
            if current_batch_size == train_batch_size:
                # optimize model
                gradient_loss, gradient_smaller_equal, gradient_smaller_strict, gradient_larger_equal, gradient_larger_strict, gradient_sum_constraint_food_expense, gradient_sum_constraint_income, train_loss_value, train_metric_value, train_satisfaction_ratio, _ \
                    = compute_sum_grad_undo_norm(model=model,
                                                 inputs=x,
                                                 targets=y,
                                                 batch_size=current_batch_size,
...
def train_model_unconstrained_only_label(model, optimizer, train_number, train_dataset, train_batch_size, val_number, val_dataset,
                                         ...
                                         lower_bound_output_variables, upper_bound_output_variables, lower_bound_input_variables,
                                         upper_bound_input_variables):
    """Train model without constraints semi-suprevised. The batches that are unsupervised are not used for training.
...
def train_model_fuzzy(model, input_dim, output_dim, optimizer, list_bound_constraints, train_number, train_dataset, train_batch_size,
                      val_number, val_dataset, val_batch_size, max_epochs, learning_rate,
                      max_constant_perf, save_location_model, save_file_model,
                      lower_bound_output_variables, upper_bound_output_variables, lower_bound_input_variables,
                      upper_bound_input_variables):
    """Train model with fuzy-logic loss function.
...
def train_model_semi_sup_fuzzy(model, input_dim, output_dim, optimizer, list_bound_constraints, train_number, train_dataset,
                               train_batch_size, val_number, val_dataset, val_batch_size, max_epochs, learning_rate, save_location_model,
                               save_file_model, lower_bound_output_variables, upper_bound_output_variables, lower_bound_input_variables,
                               upper_bound_input_variables, sup_batches):
    """Train model semi-supervised with fuzzy-logic loss function.
