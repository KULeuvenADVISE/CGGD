...
import pandas as pd
import FamilyIncomeCGGD as cggd
import FamilyIncomeConstraints as fic
...
def perform_experiment(directory_dataset, name_dataset, extension_dataset, initial_learning_rate, test_size, validation_size,
                       max_num_epochs, max_epoch_constraints, batch_size, acceptable_sat_ratio, max_bumps_con, max_time_after_bumps_con,
                       max_constant_perf_con, max_restarts_con, random_state, number_dataset_realizations, number_weight_initializations,
                       name_experiment, constrained, margin, percentages_unsupervised):
    ...
    constrained : Boolean
        Boolean indicating if the experiment uses constraints or not.
    margin : tf.Tensor
    ...
    list_bound_constraints, lower_bound_unnormalized, upper_bound_unnormalized, lower_bound_income, upper_bound_income = fic.make_list_bound_constraints(margin=margin)
    ...
    dic_bound_con = {'list_bound_constraints': list_bound_constraints, 'lower_bound_unnormalized': lower_bound_unnormalized,
                     'upper_bound_unnormalized': upper_bound_unnormalized, 'lower_bound_income': lower_bound_income,
                     'upper_bound_income': upper_bound_income}
        ...
        for r in range(0, len(percentages_unsupervised)):
            ...
            for i in range(0, len(weight_states)):
                ...
                if i == 0:
                    ...
                    if not os.path.isdir(save_location_model + 'weights_con_model_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r)):
                        os.makedirs(save_location_model + 'weights_con_model_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r))

                model.save_weights(
                    filepath=save_location_model + 'weights_con_model_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r) + '/weights_con_model_initial_' + str(
                        i) + '_dataset_' + str(k) + '_percentage_' + str(r))
                ...
                if constrained:
                    trained_model, train_loss_results, train_metric_results, train_satisfaction_results, val_loss_results, val_metric_results, val_satisfaction_results, epoch, list_lr, list_lr_loss, list_lr_con, timing, best_epoch = \
                        cggd.train_model_semi_sup(model=model, input_dim=input_dim, output_dim=output_dim, optimizer=optimizer,
                                                 ...
                                                 save_location_model=save_location_model + 'weights_con_model_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r),
                                                 save_file_model='/weights_con_model_trained_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r),
                                                 sup_batches=supervised_batches, lower_bound_output_variables=lower_bound_unnormalized,
                                                 upper_bound_output_variables=upper_bound_unnormalized,
                                                 lower_bound_input_variables=lower_bound_income, upper_bound_input_variables=upper_bound_income)
                else:
                    with open(directory_experiment + '/' + name_experiment + '/' + name_dataset + '/Constrained/Aux_variables' + '_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r) + '.pickle', 'rb') as handle:
                        aux_var_con = pickle.load(handle)

                    max_num_epochs = aux_var_con['epoch']
                    # train the model without constraints
                    trained_model, train_loss_results, train_metric_results, train_satisfaction_results, val_loss_results, val_metric_results, val_satisfaction_results, epoch, list_lr_loss, timing, best_epoch = \
                        cggd.train_model_unconstrained_only_label(model=model, optimizer=optimizer, train_number=np.size(x_train, 0),
                                                      train_dataset=train_dataset, train_batch_size=batch_size, val_number=np.size(x_val, 0),
                                                      ...
                                                      save_location_model=save_location_model + 'weights_uncon_model_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r),
                                                      input_dim=input_dim, output_dim=output_dim, 
                                                      list_bound_constraints=list_bound_constraints,
                                                      save_file_model='/weights_uncon_model_trained_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r),
                                                      sup_batches=supervised_batches,
                                      lower_bound_output_variables=lower_bound_unnormalized,
                                      upper_bound_output_variables=upper_bound_unnormalized, lower_bound_input_variables=lower_bound_income,
                                      upper_bound_input_variables=upper_bound_income)
                ...
                with open(save_location_training + '_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r) + '.pickle', 'wb') as handle:
                ...
                with open(save_location_lr + '_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r) + '.pickle', 'wb') as handle:
                ...
                with open(save_location_aux_variables + '_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r) + '.pickle', 'wb') as handle:
...
def perform_fuzzy_experiment(directory_dataset, name_dataset, extension_dataset, initial_learning_rate, test_size, validation_size, max_num_epochs,
                             max_epoch_constraints, batch_size, acceptable_sat_ratio, max_bumps_con, max_time_after_bumps_con, max_constant_perf_con,
                             max_restarts_con, random_state, number_dataset_realizations, number_weight_initializations, name_experiment, margin,
                             percentages_unsupervised):
    """ Perform the experiment for the dataset at directory_experiment + name_experiment + extension_experiment.
        ...
        for r in range(0, len(percentages_unsupervised)):
            ...
            for i in range(0, len(weight_states)):
                ...
                if i == 0:
                    ...
                    if not os.path.isdir(save_location_model + 'weights_fuzzy_model_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r)):
                        os.makedirs(save_location_model + 'weights_fuzzy_model_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r))

                model.save_weights(filepath=save_location_model + 'weights_fuzzy_model_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r) + '/weights_fuzzy_model_initial_' + str(i) + '_dataset_' + str(k))
                ...
                trained_model, train_loss_results, train_metric_results, train_satisfaction_results, val_loss_results, val_metric_results, val_satisfaction_results, epoch, list_lr_loss, timing, best_epoch = \
                    cggd.train_model_semi_sup_fuzzy(model=model, optimizer=optimizer, train_number=np.size(x_train, 0),
                                          train_dataset=train_dataset, train_batch_size=batch_size, val_number=np.size(x_val, 0),
                                          val_dataset=val_dataset, val_batch_size=batch_size, max_epochs=max_num_epochs,
                                          learning_rate=initial_learning_rate,
                                          save_location_model=save_location_model + 'weights_fuzzy_model_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r),
                                          input_dim=input_dim, output_dim=output_dim, 
                                          list_bound_constraints=list_bound_constraints,
                                          save_file_model='/weights_fuzzy_model_trained_' + str(i) + '_dataset_' + str(k) + '_percentage_' + str(r),
                                                   sup_batches=supervised_batches,
                                                   lower_bound_output_variables=lower_bound_unnormalized,
                                                   upper_bound_output_variables=upper_bound_unnormalized,
                                                   lower_bound_input_variables=lower_bound_income,
                                                   upper_bound_input_variables=upper_bound_income)
                ...
                if unconstrained:
                    ...
                    trained_model, train_loss_results, train_metric_results, train_satisfaction_results, val_loss_results, val_metric_results, val_satisfaction_results, epoch, list_lr_loss, timing, best_epoch = \
                        cggd.train_model_unconstrained(model=model, optimizer=optimizer, train_number=np.size(x_train, 0),
                                                      ...
                                                      val_dataset=val_dataset, val_batch_size=batch_size, max_epochs=max_num_epochs,
                                                      learning_rate=initial_learning_rate, max_constant_perf=2 * max_num_epochs,
                                                      save_location_model=save_location_model + 'weights_uncon_model_' + str(i) + '_dataset_' + str(k) + '_size_' + str(train_size[n]),
                                                      input_dim=input_dim, output_dim=output_dim, 
                                                      list_bound_constraints=list_bound_constraints,
