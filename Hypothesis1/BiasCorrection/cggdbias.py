...
def compute_sum_grad(model, inputs, targets, batch_size, output_dim, neur_act, bound_sm_eq, bound_sm_st, bound_la_eq, bound_la_st,
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        unsatisfied_sum = perform_satisfaction_expression(satisfaction_signed_indicator, tape_pred)
    ...
    return grad_loss, grad_sm_eq, grad_sm_st, grad_la_eq, grad_la_st, grad_sum, loss_value, metric_value, sat_ratio


def unsup_compute_sum_grad(model, inputs, targets, batch_size, output_dim, neur_act, bound_sm_eq, bound_sm_st, bound_la_eq, bound_la_st,
                           ind_sm_eq, ind_sm_st, ind_la_eq, ind_la_st, signed_indicator):
    ...
    return grad_sm_eq, grad_sm_st, grad_la_eq, grad_la_st, grad_sum, sat_ratio


def compute_fuzzy_grad(model, inputs, targets, batch_size, output_dim, neur_act, bound_sm_eq, bound_sm_st, bound_la_eq, bound_la_st, ind_sm_eq,
    ...
    the unconstrained model and the S2G2 model.
...
def rescale_sum_grad(grad_loss, grad_sm_eq, grad_sm_st, grad_la_eq, grad_la_st, grad_sum, neur_act, rescale_constant):
    ...
    for i in range(0, len(neur_act)):
        if i % 2 == 0:  # this correspond to a weight matrix
            ...
            norm_grad_sum.append(tf.math.l2_normalize(grad_sum[i], axis=1))

            layer_max = tf.cast(tf.norm(tf.math.abs(grad_loss[i]), axis=1), dtype=tf.float32)
            scale_layer_max = tf.math.scalar_mul(rescale_constant, layer_max)  # rescale with rescale factor
            ...
            sign_grad_sum = tf.cast(tf.math.sign(grad_sum[i]), dtype=tf.float32)
            norm_grad_sum.append(sign_grad_sum)
    ...
    for i in range(0, len(neur_act)):
        if i % 2 == 0:
            ...
            scale_grad_sum.append(tf.math.multiply(resh_max_layer, norm_grad_sum[i]))
        else:
            ...
            scale_grad_sum.append(tf.math.multiply(max_layers[i], norm_grad_sum[i]))
...
def unsup_rescale_sum_grad(grad_sm_eq, grad_sm_st, grad_la_eq, grad_la_st, grad_sum, neur_act, max_layers):
    """Rescale gradients of constraints according to the gradient of the loss. In addition, the rescaled gradients are all combined into a
    single gradient that needs to be applied.
    ...
    while epoch - epoch_constraints < max_epochs and epoch_constraints < max_con_epochs:
        ...
        for x, y in train_dataset:
            ...
            if current_batch_size == train_batch_size:
                # optimize model
                gradient_loss, gradient_smaller_equal, gradient_smaller_strict, gradient_larger_equal, gradient_larger_strict, gradient_sum_constraint, train_loss_value, train_metric_value, train_satisfaction_ratio \
                ...
                rescaled_gradients, max_layers = rescale_sum_grad(grad_loss=gradient_loss,
                                                                  ...
                                                                  grad_sum=gradient_sum_constraint,
                                                                  neur_act=neur_act,
